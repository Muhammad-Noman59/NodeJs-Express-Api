...
const loginUser = asyncHandler(async (req, res,) =>{
    ...
    // fi need email or username if(!(username || email)){}
...
const changeCurrentPassword =asyncHandler(async (req, res) => {
    ...
    const isPasswordCorrect = await user.isPsswordCorrect(oldPassword)
...
const updateAcaoutnDetails = asyncHandler(async (req, res)=>{

    const {fullName, email, } = req.body

    if(!fullName || !email){
        throw new apiError(400, "All fieled are required")
    }
...
const updateUserAvatar = asyncHandler(async(req, res)=>{
    ...
    .status(200)
    .json(
        new apiResponse(200, user, " Avatar image upadeted successfully")
    )
})
...
const updateUserCoverImage = asyncHandler(async(req, res)=>{
    ...
    return res
            .status(200)
            .json(
                new apiResponse(200, user, " Cover image upadeted successfully")
            )
